...
class TimeWatch:
    ...
    def edit_date(self, year, month, date, date_duration):
        ...
        if isinstance(current_date_duration_hour, str):  # if there is no punch at att, set the var to 100
            current_date_duration_hour = 0
            current_date_duration_min = 0

        # Scenario 1: The duration is above 9+ hours, no need to update
        if current_date_duration_hour >= 9 and current_date_duration_min >= 0:
        ...
        # Scenario 4: The duration is below 9 hours, but two punches were made. Below 11AM
        elif date_duration[date][2] and date_duration[date][4] == 'punched_no_change' and \
                date_duration[date][0][0] < 9 and date_duration[date][1][0] < 11:
            # Calculating how many hours we need
            delta1 = datetime.timedelta(hours=9, minutes=5 + jitter)
        ...
        # Scenario 5: The duration is below 9 hours, but two punches were made. Above 11AM
        elif date_duration[date][2] and date_duration[date][4] == 'punched_no_change' and \
                date_duration[date][0][0] < 9 and date_duration[date][1][0] >= 11:
            ...
            delta2 = datetime.timedelta(hours=date_duration[date][0][0], minutes=date_duration[date][0][1])
            how_many_left = delta1 - delta2
            punch = datetime.timedelta(hours=date_duration[date][1][0], minutes=date_duration[date][1][1]) - how_many_left
            hour_start = int(punch.seconds / 3600)
            minutes_start = int((punch.seconds / 60) % 60)

            what_to_punch = 1234  # punch in position 1,2,3,4
            start = hour_start, minutes_start
            if date_duration[date][1][1] == 0:
                end = date_duration[date][1][0] - 1, 59  # edge case where time ends with :00
            else:
                end = date_duration[date][1][0], date_duration[date][1][1] - 1

            start1 = date_duration[date][1][0], date_duration[date][1][1]
            end1 = date_duration[date][3][0], date_duration[date][3][1]

        # Scenario 6: not punched at all.
            ...
            if isinstance(date_duration[date][1][0], int):
                ...
                start = 9, 0 + jitter
                end = 18, 5 + jitter + random.randint(0, 5)
...
*MANDATORY* - login to <https://checkin.timewatch.co.il/punch/punch2.php|timewatch_site> and check my work.
In case you forgot, the company number is 2391"""